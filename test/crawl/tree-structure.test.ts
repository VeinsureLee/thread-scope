 ...
 *
 * 注意：所有名称与 ID 均为合成测试数据，不包含真实论坛内容。
 */
...
describe("论坛树结构 — 节点识别", () => {
  ...
  describe("extractHref（从 t 字段 HTML 提取 href）", () => {
    it('提取 /board/xxx href', () => {
      expect(extractHref('<a href="/board/b1">版块一</a>'))
        .toBe("/board/b1");
    });

    it('提取 /section/xxx href', () => {
      expect(extractHref('<a href="/section/sec-a">分区甲</a>'))
        .toBe("/section/sec-a");
    });
  ...
  describe("isBoardHref / isSectionHref（href 路径判断）", () => {
    it('/board/xxx → 版块 (leaf)', () => {
      expect(isBoardHref("/board/b1")).toBe(true);
      expect(isBoardHref("/board/b2")).toBe(true);
      expect(isBoardHref("/board/b3")).toBe(true);
    });

    it('/section/xxx → 分区 (branch)', () => {
      expect(isSectionHref("/section/sec-a")).toBe(true);
      expect(isSectionHref("/section/zone-b")).toBe(true);
    });

    it('/board/ 不是分区', () => {
      expect(isSectionHref("/board/b1")).toBe(false);
    });

    it('/section/ 不是版块', () => {
      expect(isBoardHref("/section/sec-a")).toBe(false);
    });
  ...
  describe("extractBoardEname（从 href 提取英文名）", () => {
    it('提取 /board/b1 → b1', () => {
      expect(extractBoardEname("/board/b1")).toBe("b1");
    });

    it('多级路径只取第一段', () => {
      expect(extractBoardEname("/board/b2/sub")).toBe("b2/sub");
    });

    it('无 /board/ 返回空', () => {
      expect(extractBoardEname("/section/sec-a")).toBe("");
    });
  });
  ...
  describe("extractName（从 t 字段 HTML 提取名称）", () => {
    it('提取 a 标签文本', () => {
      expect(extractName('<a href="/section/zone-b">分区乙</a>'))
        .toBe("分区乙");
      expect(extractName('<a href="/board/b1">版块一</a>'))
        .toBe("版块一");
    });
...
describe("论坛树结构 — 递归逻辑", () => {
  ...
  it("全部是版块 → boards 收集，sections 为空", () => {
    const entries: MockEntry[] = [
      { t: '<a href="/board/b1">版块A</a>', id: "" },
      { t: '<a href="/board/b2">版块B</a>', id: "" },
      { t: '<a href="/board/b3">版块C</a>', id: "" },
    ];
    const { sections, boards } = classifyNodes(entries);
    expect(boards).toHaveLength(3);
    expect(sections).toHaveLength(0);
    expect(extractBoardEname(extractHref(entries[0]!.t))).toBe("b1");
  });

  it("混合分区和版块 → 正确分离", () => {
    const entries: MockEntry[] = [
      { t: '<a href="/section/sec-a">子分区A</a>', id: "sec-a" },
      { t: '<a href="/board/b1">版块A</a>', id: "" },
      { t: '<a href="/board/b2">版块B</a>', id: "" },
