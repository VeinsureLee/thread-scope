...
export async function searchThreads(
  ...
  for (const ename of scope.boards) {
    try {
      hits.push(...(await searchBoardArticles(ename, keyword, opts, repos.searchRepo)));
    } catch {
      // 版面搜索失败不影响其他（沿用 traffic 的错误隔离策略）
    }
  }
  ...
  const result: SearchThreadHit[] = [];
  for (const hit of limited) {
    const url = hit.row.url;
    const board = boardFromArticleUrl(url);
    const articleId = articleIdFromUrl(url);
    if (!board || !articleId) continue;

    try {
      const detail = await fetchThreadDetail(board, articleId, { maxPages: opts.maxThreadPages }, repos.threadRepo);
      result.push({
        boardEname: board,
        articleId,
      ...
      // 正文抓取失败不影响其他命中（沿用 traffic 的错误隔离策略）
    }
  }

  return { scope, hits: result };
}
